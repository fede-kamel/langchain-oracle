...
| [`@oracle/langgraph-oracledb`](./libs/js/langgraph-oracledb) | [![npm](https://img.shields.io/npm/v/%40oracle%2Flanggraph-oracledb?label=%20&logo=npm&logoColor=white&color=CB3837)](https://www.npmjs.com/package/@oracle/langgraph-oracledb) | `npm install @oracle/langgraph-oracledb` | **LangGraph.js persistence on Oracle Database.** TypeScript/JavaScript checkpointer (`OracleCheckpointSaver`) and key/value store (`OracleStore`) with optional vector search for durable graph state and long-term agent memory. |
